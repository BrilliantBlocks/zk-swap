...
func __setup__{syscall_ptr : felt*, range_check_ptr, pedersen_ptr : HashBuiltin*}():
    ...
    ISellPool.setPoolParams(pool_address, POOL_PARAMS)
...
func test_initialization_ERC_contracts{syscall_ptr : felt*, range_check_ptr, pedersen_ptr : HashBuiltin*}():
    ...
    let (erc20_balance_token_buyer) = IERC20.balanceOf(erc20_contract_address, NFT_BUYER)
    let (erc20_balance_token_owner) = IERC20.balanceOf(erc20_contract_address, POOL_AND_NFT_OWNER)
...
func test_getPoolConfig_with_expected_output{syscall_ptr : felt*, range_check_ptr, pedersen_ptr : HashBuiltin*}():
    ...
    let (pool_factory) = ISellPool.getPoolFactory(pool_address)
    let (pool_params: PoolParams) = ISellPool.getPoolConfig(pool_address)

    let (high, low) = split_felt(pool_address)
    let pool_address_token = Uint256(low, high)
    let (pool_owner) = IMintPool.ownerOf(pool_factory_contract_address, pool_address_token)
