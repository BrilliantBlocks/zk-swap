...
func test_exponential_curve_with_expected_output{syscall_ptr : felt*, range_check_ptr, pedersen_ptr : HashBuiltin*}():
    ...
    let (total_price) = IBondingCurve.getTotalPriceV2(exponential_curve_contract_address, NUMBER_TOKENS, CURRENT_PRICE, DELTA)
    let (new_price) = IBondingCurve.getNewPriceV2(exponential_curve_contract_address, NUMBER_TOKENS, CURRENT_PRICE, DELTA)
